...
<div class="card">
    <?= $this->Form->create($attendance, ['class' => 'form-horizontal', 'type' => 'post']) ?>
    ...
    <div class="card-body card-block">
        <div class="row form-group">
            ...
            <?= $this->Form->control('site_id', ['options' => $sites, 'label' => false, 'div' => false, 'data-placeholder' => 'Choose site', 'class' => 'standardSelect', 'tabindex' => '1', 'required' => true, 'empty' => true]) ?>
        </div>

        <div class="row form-group">
            <div class="col col-md-3"><label for="employee_id" class="form-control-label"><?= __('Employee') ?></label></div>
            <?= $this->Form->control('employee_id', ['label' => false, 'div' => false, 'data-placeholder' => 'Choose employee', 'class' => 'standardSelect', 'tabindex' => '1', 'required' => true, 'empty' => true]) ?>
        </div>

        <div class="row form-group">
            <div class="col col-md-3"><label for="work_date" class="form-control-label"><?= __('Work Date'); ?></label></div>
            <?= $this->Form->control('work_date', ['label' => false, 'class' => 'form-control', 'placeholder' => __('Enter work date'), 'type' => 'text', 'value' => $attendance->work_date ? date('m/d/Y', strtotime($attendance->work_date)) : '']); ?>
        </div>
        ...
        <div class="row form-group">
            ...
            <?= $this->Form->control('time_in', ['label' => false, 'class' => 'form-control', 'placeholder' => __('Enter time in'), 'type' => 'text', 'value' => $attendance->time_in ? date('H:i', strtotime($attendance->time_in)) : '']); ?>
        </div>
